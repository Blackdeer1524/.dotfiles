...
local path_to_jar = vim.split(vim.fn.glob(path_to_plugins .. "/org.eclipse.equinox.launcher_*.jar"), "\n")[1]
...
vim.list_extend(
	bundles,
	vim.split(vim.fn.glob(require("lsp.defaults").MASON_PACKAGES .. "/java-test/extension/server/*.jar", 1), "\n")
)
config["init_options"] = {
	bundles = bundles,
}
